...
Use ifdef and ifndef statements to control content based on distribution, as described in the previous section. For example, the following example renders differently in OKD and OpenShift Container Platform:
...
Note that you must set and unset a temporary attribute, `restricted` in this example, for each assembly that contains text that varies between assemblies as well as use the temporary attribute in the applicable ifdef and ifndef statements.
