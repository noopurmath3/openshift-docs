[[upgrading_disconnected]]
= Upgrading in a disconnected environment
{product-author}
{product-version}
:latest-tag: v3.10.15
...
Frequently, portions of a datacenter do not have access to the Internet, even
via proxy servers. Upgrading {product-title} in these environments is
considered a disconnected upgrade.

An {product-title} disconnected upgrade differs from a regular
upgrade in two primary ways:

- The {product-title} software channels and repositories are not available via
Red Hat’s content distribution network.
...
A disconnected upgrade ensures the {product-title} software is made
available to the relevant servers, then follows the same upgrade process as
a standard connected upgrade. This topic additionally details how to
...
During the upgrade (and for later updates, should you so choose), you will
...
during the upgrade, make sure you select the *Basic Web Server* option.
...
of RHEL 7 and to perform a minimal upgrade. You will also
...
You can now follow the standard upgrade documentation to:
...
[[disconnected-post-upgrade-changes]]
...
upgrade, these images would be pulled from Red Hat’s registry on demand.
